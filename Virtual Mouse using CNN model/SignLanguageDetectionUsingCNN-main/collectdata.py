...
# Create Subdirectories for Each Alphabet Letter:
...
import os
import cv2
cap=cv2.VideoCapture(0)  #Open Webcam and Start Capturing:
while True:
    _,frame=cap.read()
    ...
    row = frame.shape[1]
    col = frame.shape[0]
    cv2.rectangle(frame,(0,40),(300,300),(255,255,255),2)  #Draw Rectangle Around Region of Interest
    
    # Displaying and Cropping Region of Interest (ROI) from Webcam Feed
    cv2.imshow("data",frame)  #Displays the video feed in a window.
    ...
    # Capture Image for Each Key Press:
    interrupt = cv2.waitKey(10)   #Waits for 10 milliseconds to capture key input.
